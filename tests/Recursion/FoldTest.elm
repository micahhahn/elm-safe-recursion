...
foldMapListCount =
    runRecursion <|
        \(ListNode _ list) ->
            foldMapList (\x state after -> recurse x ((+) state >> after)) 1 list base
...
foldDictCount =
    runRecursion <|
        \(DictNode _ dict) ->
            foldDict (\_ x accum -> x + accum) 1 dict base
...
foldMapDictCount =
    runRecursion <|
        \(DictNode _ dict) ->
            foldMapDict (\_ x accum after -> recurse x ((+) accum >> after)) 1 dict base
...
foldMapArrayCount =
    runRecursion <|
        \(ArrayNode _ array) ->
            foldMapArray (\x accum after -> recurse x ((+) accum >> after)) 1 array base
